...
def get_data_from_api(request_url,service_ident, params_diction, expire_in_days=0.5):
    ...
    if data:
        ...
        temp_diction=deepcopy(params_diction)
        blog_identifier=temp_diction.pop('blog_identifier')
        resp = oauth_inst.get(request_url.format(blog_identifier),params=temp_diction)
...
def to_csv(data,flag):
    if flag=='followings':
        f=open('followings.csv','w',encoding='utf-8',newline='')
        ...
        if 'errors' not in data:
            for record in data['response'].get('blogs'):
                row={}
                row['blog_title']=record['title'].strip()
                row['unique_name']=record['name'].strip()
                row['url']=record['url'].strip()
                row['decription']=record['description'].strip()
                writer.writerow(row)
        f.close()
    else:
        f=open('followers.csv','w',encoding='utf-8',newline='')
        writer=csv.DictWriter(f,fieldnames=['unique_name','url','recent_post'],\
                                  extrasaction='ignore',delimiter=',',quotechar='"')
        writer.writeheader()
        if 'errors' not in data:
...
def main(username=None,limit=100):
    ...
    if not REQUEST_TOKEN_URL or not BASE_AUTH_URL:
        ...
        exit()
    if username==None:
        username=input("please enter your Tumblr username for me to generate your blog identifier: ").lower()
    tumblr_search_baseurl1='https://api.tumblr.com/v2/blog/{}/following'
    tumblr_search_baseurl2='https://api.tumblr.com/v2/blog/{}/followers'
    # Invoke functions
    tumblr_search_params = {'blog_identifier':'{}.tumblr.com'.format(username), "limit":limit}
